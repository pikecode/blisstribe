  ...
  <view v-if="activity" class="activity-detail">
    ...
    <view class="activity-detail__main">
      ...
      <view class="activity-detail__stats">
        <text>{{ activityRegistrationStateText(activity.registrationStatus) }}</text>
    ...
    <view class="activity-detail__form">
      ...
      <input class="activity-detail__input" :value="name" placeholder="姓名或称呼" maxlength="40" @input="onNameInput" />
      <textarea class="activity-detail__textarea" :value="message" placeholder="可以写下你关注的问题" maxlength="300" @input="onMessageInput" />
    </view>
...
import { activityApi, activityRegistrationStateText, activityRegistrationStatusText, activityTypeText, type Activity } from '@/api/modules/activity'
...
const canSubmit = computed(() => activity.value?.registrationStatus === 'registering' && !activity.value?.myRegistration)
const submitText = computed(() => {
  if (submitting.value) return '提交中...'
  if (activity.value?.myRegistration) return activityRegistrationStatusText(activity.value.myRegistration.status)
...
async function loadDetail() {
  ...
  try {
    activity.value = await activityApi.detail(activityId.value)
